...
async function getDailySummary(req, res) {
    try {
        ...
        if (isSalesPerson) {
            ...
            salesUsers = await prisma_1.prisma.user.findMany({
        ...
        for (const user of salesUsers) {
            ...
            const cashSalesTotal = cashSales.reduce((sum, sale) => sum.plus(sale.totalPaid), new library_1.Decimal(0));
            ...
            const bankSalesTotal = bankSales.reduce((sum, sale) => sum.plus(sale.totalPaid), new library_1.Decimal(0));
            // Get credit sales (sales where totalCredit > 0)
            const creditSales = await prisma_1.prisma.sale.findMany({
                where: {
                    salespersonId: user.id,
                    createdAt: { gte: startOfDayUTC, lt: endOfDayUTC },
                    totalCredit: { gt: 0 },
            ...
            const creditSalesTotal = creditSales.reduce((sum, sale) => sum.plus(sale.totalCredit), new library_1.Decimal(0));
            // Get payments received from companies (credit collections)
            const paymentsReceived = await prisma_1.prisma.paymentReceived.findMany({
                where: {
                    salespersonId: user.id,
                    createdAt: { gte: startOfDayUTC, lt: endOfDayUTC },
                },
            });
            const paymentsReceivedTotal = paymentsReceived.reduce((sum, payment) => sum.plus(payment.amount), new library_1.Decimal(0));
            ...
            const salespersonExpenses = await prisma_1.prisma.expense.findMany({
                where: {
                    OR: [
                        { salespersonId: user.id },
            ...
            const salespersonExpensesTotal = salespersonExpenses.reduce((sum, expense) => sum.plus(expense.amount), new library_1.Decimal(0));
            // Calculate totals
        ...
        const overallCashSales = summaryData.reduce((sum, d) => sum.plus(d.sales.cashSales), new library_1.Decimal(0));
        const overallBankSales = summaryData.reduce((sum, d) => sum.plus(d.sales.bankSales), new library_1.Decimal(0));
        const overallCreditSales = summaryData.reduce((sum, d) => sum.plus(d.sales.creditSales), new library_1.Decimal(0));
        const overallPayments = summaryData.reduce((sum, d) => sum.plus(d.payments.received), new library_1.Decimal(0));
        const overallExpenses = summaryData.reduce((sum, d) => sum.plus(d.expenses.total), new library_1.Decimal(0));
