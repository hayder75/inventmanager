...
exports.getBankDeposits = getBankDeposits;
...
async function createSale(req, res) {
    try {
        ...
        if (req.body.subtotal !== undefined) {
            subtotal = new library_1.Decimal(req.body.subtotal);
        }
        else {
            subtotal = new library_1.Decimal(0);
            items.forEach(item => {
                const itemTotal = new library_1.Decimal(item.finalPrice).times(item.quantity);
        ...
        if (req.body.vatAmount !== undefined) {
            vatAmount = new library_1.Decimal(req.body.vatAmount);
        }
        else {
            vatAmount = new library_1.Decimal(0);
            if (vatEnabled) {
                vatAmount = subtotal.times(0.075);
            }
        }
        if (req.body.totAmount !== undefined) {
            totAmount = new library_1.Decimal(req.body.totAmount);
        }
        else {
            totAmount = new library_1.Decimal(0);
            if (totEnabled && !vatEnabled) {
                totAmount = subtotal.times(0.03);
            }
        }
        if (req.body.totalAmount !== undefined) {
            totalAmount = new library_1.Decimal(req.body.totalAmount);
        }
        else {
            totalAmount = subtotal.plus(vatAmount).plus(totAmount);
        }
        // Calculate commission
        let commissionAmount = new library_1.Decimal(0);
        ...
        let totalCredit = new library_1.Decimal(0);
        let bankType = null;
        ...
        const sale = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                data: {
                    ...
                    items: {
                        create: items.map(item => ({
                            ...
                            subtotal: new library_1.Decimal(item.finalPrice).times(item.quantity),
                            // Surplus management fields
                            surplusAmount: item.surplusAmount ? new library_1.Decimal(item.surplusAmount) : new library_1.Decimal(0),
                            adminCutAmount: item.adminCutAmount ? new library_1.Decimal(item.adminCutAmount) : new library_1.Decimal(0),
                            adminCutPercentage: item.adminCutType === 'percentage' && item.adminCutValue ? new library_1.Decimal(item.adminCutValue) : null,
                            remainingSurplus: item.remainingSurplus ? new library_1.Decimal(item.remainingSurplus) : new library_1.Decimal(0),
                            salespersonGetsCommission: item.salespersonGetsCommission || false,
                            salespersonCommissionAmount: item.salespersonCommissionAmount ? new library_1.Decimal(item.salespersonCommissionAmount) : new library_1.Decimal(0),
                            salespersonCommissionPercentage: item.salespersonCommissionType === 'percentage' && item.salespersonCommissionValue ? new library_1.Decimal(item.salespersonCommissionValue) : null,
                        })),
                    },
                },
                include: {
            ...
            if (totalSurplusCommission > 0) {
                await tx.user.update({
                    ...
                    data: {
                        totalCommission: {
                            increment: new library_1.Decimal(totalSurplusCommission),
                        },
                    },
                });
            }
...
async function getBankDeposits(req, res) {
    try {
        ...
        if (bankType) {
            where.bankType = bankType; // Specific bank type
        }
        const sales = await prisma_1.prisma.sale.findMany({
            where,
            select: {
